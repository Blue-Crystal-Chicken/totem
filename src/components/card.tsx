...
export default function MyCard({
    ...
    return (
        <Card
            ...
            className={`group cursor-pointer w-full max-w-45 h-50 aspect-square rounded-3xl border-2 border-[#C8E8FF] 
                       hover:border-[#378ADD] hover:shadow-xl hover:-translate-y-1 
                       transition-all duration-300 active:scale-[0.97] 
                       flex flex-col items-center p-5 gap-4 bg-${bg || 'white'} overflow-hidden`}
        >
            {/* Icona */}
            {icon && (
                <div
                    className="w-16 h-16 rounded-2xl flex items-center justify-center text-4xl shadow-sm shrink-0"
                    style={{ background: bgIcon ?? "#EBF5FF" }}
                >
                    {icon}
                </div>
            )}
            ...
            {/* Immagine Prodotto */}
            {image && !icon && (
                <div className="relative w-full h-24 aspect-square rounded-2xl overflow-hidden shrink-0">
                    <img
                        src={image}
                        alt={title}
