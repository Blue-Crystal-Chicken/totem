...
export default function CardMenuDetails({ menu }: MenuDetailsProps) {

    const imageUrl = getImageUrl(menu.imgPath ?? undefined);
            ...
            <div className="flex flex-col h-full min-h-0 bg-white">
                ...
                {/* Nome + Prezzo */}
                    ...
                    <div className="grid grid-cols-1 gap-2.5">
                        {menu.menuProducts.map((prod) => (
                                <CardOfferProduct 
                                name={prod.productName}
                                price={prod.unitPrice}
                                quantity={prod.quantity}
                                imgPath={getImageUrl(prod.productimgPath)}   
                                />
                            ))}
                        </div>
                </div>
                            ...
                            <div className="shrink-0 border-t border-[#C8E8FF] bg-gray-100 backdrop-blur-md px-4 py-4">
                                ...
                                <div className="flex flex-col items-center justify-center mt-3 w-full gap-4">
                                    ...
                                    >
                                        ...
                                        <h1 className="font-bold text-[#185FA5] text-4xl tabular-nums">€ {(menu.price * quantity).toFixed(2)}</h1>
                                        </div>
                                        </div>
                                </div>
